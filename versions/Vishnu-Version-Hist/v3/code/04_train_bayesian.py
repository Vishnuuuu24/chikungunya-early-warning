#!/usr/bin/env python3
"""
Experiment 04: Train Bayesian State-Space Model (Stabilized)

Phase 4.2: Stabilization of MCMC diagnostics.

Changes from v2_proto:
- Increased warmup: 300 -> 1000
- Increased samples: 300 -> 1000
- Increased chains: 2 -> 4
- Added adapt_delta=0.95 for fewer divergences
- Updated Stan model with phi constraint

This script:
1. Loads processed features
2. Selects ONE CV fold (fold_2019)
3. Fits the Bayesian model with stabilized settings
4. Reports MCMC diagnostics
5. Generates posterior predictive check
...
def main():
    parser = argparse.ArgumentParser(description="Train Bayesian state-space model (stabilized)")
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        "--n-warmup",
        type=int,
        default=1000,  # STABILIZATION: increased from 300
    ...
    parser.add_argument(
        "--n-chains",
        type=int,
        default=4,  # STABILIZATION: increased from 2
        help="Number of MCMC chains"
    )
    parser.add_argument(
        "--adapt-delta",
        type=float,
        default=0.95,  # STABILIZATION: higher acceptance rate
    ...
    print("=" * 60)
    print("CHIKUNGUNYA EWS - BAYESIAN MODEL (Phase 4.2 Stabilized)")
    print("=" * 60)
    ...
    print(f"adapt_delta: {args.adapt_delta}")
    ...
    model_config = {
        ...
        'adapt_delta': args.adapt_delta,  # STABILIZATION: new parameter
        'seed': 42
    }
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("PHASE 4.2 DIAGNOSTICS COMPLETE")
        print("=" * 60)
        ...
        print(f"  - phi lower bound: 0.1")
        print(f"  - rho prior: normal(0.7, 0.10)")

...
def compute_variance_spike_ratio(
    ...
    # Baseline variance (longer window, shifted to exclude current)
    df['_var_baseline'] = df.groupby(group_cols)[value_col].transform(
        lambda x: x.shift(current_window).rolling(window=baseline_window, min_periods=10).var()
    )
    
    # Mean of baseline variance
    df['_var_baseline_mean'] = df.groupby(group_cols)['_var_baseline'].transform(
        lambda x: x.rolling(window=baseline_window, min_periods=10).mean()
    )
    
    # Spike ratio
    epsilon = 0.01  # Prevent division by zero
    df['feat_var_spike_ratio'] = df['_var_current'] / (df['_var_baseline_mean'] + epsilon)
    
    # Cleanup
    df = df.drop(columns=['_var_current', '_var_baseline', '_var_baseline_mean'])

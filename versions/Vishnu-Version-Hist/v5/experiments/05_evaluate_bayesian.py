...
def get_stan_model_path() -> Path:
    """Get path to v3 Stan model file."""
...
def compute_outbreak_probability(
    ...
    Uses posterior predictive P(cases > p75_train) as outbreak probability.
    This matches the label definition used in v1.1.
...
def evaluate_single_fold(
    ...
    try:
        ...
        # P(outbreak) = P(predicted_cases > p75_threshold)
        y_rep = model.get_posterior_predictive()  # Shape: (n_draws, N_train)
        ...
        # Get training case threshold (p75)
        train_cases = train_df['cases'].values
        nonzero = train_cases[train_cases > 0]
        threshold = np.percentile(nonzero, 75) if len(nonzero) > 0 else 1
        ...
        # Compute metrics
        metrics = compute_all_metrics(y_true_test, y_pred_proba_test, threshold=0.5)
